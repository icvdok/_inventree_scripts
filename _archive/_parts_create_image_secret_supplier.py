...
for index, row in df.iterrows():
    ...
    try:
        ...
        print(f"Row {index} - Supplier PK: {row['supplier_pk']}, Supplier Part Number: {row['supplier_part_number']}, Supplier Link: {row['supplier_link']}")
        ...
        if row['supplier_pk'] and row['supplier_part_number'] and row['supplier_part_number'] != '0':
            ...
            print(f"Supplier data: {supplier_data}")  # Debugging line

            supplier_part = SupplierPart.create(api, supplier_data)
            print(f"Added supplier part: {supplier_part.SKU} for part: {part.name}")
