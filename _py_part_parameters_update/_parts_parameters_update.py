...
if not url:
    logging.error("BASE_URL is not set in the environment variables.")
    exit(1)
...
def get_parameters_templates_by_category(category_pk):
    ...
    if response.status_code == 200:
        ...
        logging.error(f"Failed to retrieve parameters: {response.status_code} - {response.text}")
        return None
...
def get_parts_by_category(category_pk):
    ...
    if response.status_code == 200:
        ...
        logging.error(f"Failed to retrieve parts: {response.status_code} - {response.text}")
        return None
